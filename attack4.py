...
def find_optimal_mapping(data_c, data_z):
    ...
    for i, key_c in enumerate(keys_c):
        for j, key_z in enumerate(keys_z):
            ...
            cost_matrix[i, j] = freq_diff + cdf_diff + sort_diff
...
for il in index_list:
    ...
    out = 'F:/Desktop/text/20250116/ours/'+ str(il)+ 'q2010 output of A4/'
    ...
    for file_name in sorted(file_list):
        if file_name[:-4] not in lists:
            ...
            filePathPlain = "F:/Desktop/text/2015/PUDF_base1_1q2015.csv"
            matrixP = read_csv_to_matrix(filePathPlain)
            ...
            matrix_cipher = generate_submatrix(matrix, selected_columns)
            matrix_plain = generate_submatrix(matrixP, selected_columns)
            ...
            keyword_count_stay = count_keywords(generate_submatrix(matrix_cipher,[
                'Length of stay'
                # ,'Age', 'Admission Type', 'Risk'
                # ,'Discharge', 'Gender', 'Race'
                ])[1:]) 

            keyword_count_aar = count_keywords(generate_submatrix(matrix_cipher,[
                # 'Length of stay',
                'Age', 'Admission Type', 'Risk'
                # ,'Discharge', 'Gender', 'Race'
                ])[1:]) 
            ...
            matrix_cipher_det = generate_submatrix(matrix_cipher, selected_columns_det)
            matrix_plain_det = generate_submatrix(matrix_plain, selected_columns_det)
            ...
            matrix_cipher_od = generate_submatrix(matrix_cipher, selected_columns_od)  # 经过OPE和DET后恢复了多少个记录
            matrix_recovered = replace_values_with_none(matrix_cipher_od[1:], value_mapping_od)
            ...
            unique_values_cipher_od = find_unique_value(list_cipher_od)
            unique_values_plain_od = find_unique_value(list_plain_od)
