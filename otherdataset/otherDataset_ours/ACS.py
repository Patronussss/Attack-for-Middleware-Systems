...
for ind in base:
    ...
    keywords_count = []
    for _ in range(50):
        print(f"-----Experiment {_+1} for {ind}-----")
        matrix_cipher = functions.random_extract(matrix, ind)
        value_mapping = {}
        
        selected_columns_plain_for_attr_recovery = ['VALUEH', 'AGE', 'SEX', 'MARST', 'RACE', 'EDUC']
        ...
        mapping_det, time_det, accuracy_det, keyword_count_det, count_det = EnhancedFrequencyAnalysisAttack(matrix_cipher, matrix_plain, selected_columns_det,"ACS",[1,1])
        ...
        specific_columns = []
        specific_target_cols = ['AGE', 'SEX', 'MARST', 'RACE', 'EDUC']
        for cipher_col, plain_col in attr_mapping.items():
        ...
        mapping, totalTime, accuracy, keyword_count_sse = AttackUsingAuxiliaryWeight(matrix_cipher_sse_after_mapping, matrix_plain, selected_columns_sse, "ACS", [5, 60, 35])
