...
def ope_attack_automatic(matrix_cipher, matrix_plain, ope_columns, attr_mapping):
    value_mapping = {}
    for cipher_col in ope_columns:
        plain_col = attr_mapping.get(cipher_col, cipher_col)
        column_cipher = functions.extract_columns(matrix_cipher, cipher_col)
        column_plain = functions.extract_columns(matrix_plain, plain_col)

        keyword_count_cipher = len(set(column_cipher))
        keyword_count_plain = len(set(column_plain))
...
selected_columns_plain_for_attr_recovery = ['VALUEH', 'AGE', 'SEX', 'MARST', 'RACE', 'EDUC', 'OCC', 'IND']
...
for ind in base:
    ...
    for _ in range(5):
        ...
        ope_target_cols = ['VALUEH', 'AGE']
        det_target_cols = ['SEX', 'MARST', 'RACE', 'EDUC']
        sse_target_cols = ['OCC', 'IND']
        
        ope_columns = [cipher_col for cipher_col, plain_col in attr_mapping.items() if plain_col in ope_target_cols]
        det_columns = [cipher_col for cipher_col, plain_col in attr_mapping.items() if plain_col in det_target_cols]
        sse_columns = [cipher_col for cipher_col, plain_col in attr_mapping.items() if plain_col in sse_target_cols]
        
        selected_columns = ope_columns + det_columns + sse_columns
        ...
        value_mapping = ope_attack_automatic(matrix_cipher, matrix_plain, ope_columns, attr_mapping)

        mapping_det, time_det, accuracy_det, keyword_count_det, count_det = FrequencyAnalysisAttack(matrix_cipher, matrix_plain, det_columns)
        value_mapping.update(mapping_det)
        
        selected_column_sse = sse_columns
        ...
        all_keywords = []
        for sse_col in selected_column_sse:
            plain_col = attr_mapping.get(sse_col, sse_col)
            plain_data = functions.extract_columns(matrix_plain, sse_col)
            cipher_data = functions.extract_columns(matrix_cipher, sse_col)
            keyword_list = list(set(plain_data).union(set(cipher_data)))
            ...
            for value in keyword_list:
                ...
                if os.path.exists(csv_file_path):
                    ...
                    frequency_dict[value] = value_dict

            freq_cipher_dict = {key: value for key, value in frequency_dict.items() if key in cipher_data}
            freq_plain_dict = {key: value for key, value in frequency_dict.items() if key in plain_data}

            volume_cipher_dict = volumn_cipher_sse[sse_col]
            volume_plain_dict = volumn_plain_sse[sse_col]

            freq_cipher_inPeriod, freq_cipher_Total = functions.numToFreqency(freq_cipher_dict)
            freq_plain_inPeriod, freq_plain_Total = functions.numToFreqency(freq_plain_dict)
        ...
        for sse_col in selected_column_sse:
            ...
            for key1 in set(cipher_data):
                if key1 in volume_cipher_dict and key1 in freq_cipher_Total:
                    volume1 = volume_cipher_dict[key1]
                    freq1 = freq_cipher_Total[key1]
                    min_score = 100
                    for key2 in set(cipher_data):
                        if key2 != key1 and key2 in volume_cipher_dict and key2 in freq_cipher_Total:
                            volume2 = volume_cipher_dict[key2]
                            freq2 = freq_cipher_Total[key2]
        ...
        for key1, value in sorted_dis_dict_.items():
            if key1 in volume_cipher_dict and key1 in freq_cipher_Total:
                ...
                min_score = 100
                candidate = key1
                for key2 in set(plain_data):
                    if key2 in volume_plain_dict and key2 in freq_plain_Total:
                        volume2 = volume_plain_dict[key2]
                        freq2 = freq_plain_Total[key2]
                        v = abs(volume1 - volume2) * alpha
                        f = abs(freq1 - freq2) * (1 - alpha)
                        score = v + f
                        if score < min_score:
