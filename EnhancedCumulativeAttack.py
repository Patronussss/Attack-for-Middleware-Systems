...
if __name__ == '__main__':
    ...
    with open(out, 'w', encoding='utf-8') as f:
        for i in base:
            ...
            for _ in range(50):
                ...
                mapping, totalTime, accuracy, keyword_count = EnhancedCumulativeAttack(matrix_cipher, matrix_plain, selected_columns_ope_withoutid, [0.5, 0.5, 0])
    ...
    # root = "dataset/text_508029.csv"
    # filePathPlain = "dataset/2015.csv"
    # out = 'result/output of CumulativeAttack-weight.txt'
    # matrix_plain = functions.read_csv_to_matrix(filePathPlain)
    ...
    #                     mapping, totalTime, accuracy, keyword_count = EnhancedCumulativeAttack(matrix_cipher, matrix_plain, selected_columns_ope_withoutid, weight=[0.5, 0.1, 0.4])
    #                     total_times.append(totalTime)
    #                     accuracies.append(accuracy)
    #                     keyword_counts.append(keyword_count)
    ...
    #                 print(avg_accuracy)
    #                 # 写入结果
    #                 f.write(f"文件路径: 4q2010\\text {weight}.txt, 执行时间: {avg_time}秒, 准确率: {avg_accuracy}, 关键字数量: {avg_count}, 实际恢复: {len(mapping) * avg_accuracy}\n")
    #                 f.write("-" * 50 + "\n")
    #             else:
    #                 continue
