...
# def euclidean_distance(dict1, dict2):
#     # 获取两个字典键的并集
#     keys_set = set(dict1.keys()).union(dict2.keys())

#     # 处理字典长度不等的情况，进行填充
#     if len(dict1)!= len(dict2):
#         new_dict1 = {k: dict1.get(k, 0) for k in keys_set}
#         new_dict2 = {k: dict2.get(k, 0) for k in keys_set}
#         dict1 = new_dict1
#         dict2 = new_dict2

#     # 提取字典1的值并从大到小排序
#     values1 = sorted(dict1.values(), reverse=True)
#     # 提取字典2的值并从大到小排序
#     values2 = sorted(dict2.values(), reverse=True)

#     # 计算这两个列表之间的欧氏距离
#     distance = math.sqrt(sum((v1 - v2) ** 2 for v1, v2 in zip(values1, values2)))
...
for il in index_list:
    ...
    out = 'F:/Desktop/text/output of A2/2/'
    ...
    for file_name in sorted(file_list):
        if file_name[:-4] not in lists:
            ...
            filePathPlain = "F:/Desktop/text/2015/PUDF_base1_1q2015.csv" # auxiliary
            # filePathPlain = os.path.join(base,file_name) # original
            ...
            # length = len(matrix)-1
            # 构建明文数据库和密文数据库
            # matrix_temp = random_submatrix(generate_submatrix(matrix, selected_columns_cipher),500)
            # matrix_plain = random_submatrix(generate_submatrix(matrixP, selected_columns_plain), 500)
    ...
    outputPath = os.path.join(out, f"{str(il)}q2010_new1.txt")
    print(outputPath)
    write_dicts_to_txt(column_count, recovered_count,OPEdict, DETdict, outputPath) 